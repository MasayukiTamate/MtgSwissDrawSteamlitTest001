...
*   **機能**:
    ...
    *   **ラウンド進行 (`start_new_round`)**:  現在の最も重要なロジックです。全プレイヤーを勝ち点順にソートし、上から順に対戦を組む簡易マッチングアルゴリズムを実装しています。
    *   **データ提供 (`get_standings_df`)**: UIが表示しやすい形式（pandas DataFrame）にデータを整形して渡します。
